...
```{r split by award, include = TRUE, results = "asis"}
vars_to_keep <- names(coef(mdl_with_controls))
labels <- vars_to_keep %>% 
    str_replace_all("type_of_sale", "Type of Sale: ") %>% 
    str_replace_all("_", " ") %>% 
    str_to_title()
condo_fe <- c("Condo Level Fixed Effects", "Yes", "Yes", "Yes", "Yes")
year_month_fe <- c("Year and Month Fixed Effects", "Yes", "Yes", "Yes", "Yes")
col_labs <- c("Certified", "Gold", "Gold Plus", "Platinum")
stargazer(certified_mdl, gold_mdl, gold_plus_mdl, plat_mdl, 
          type = "latex", title = "Heterogeneous Effects of Different Awards",
          keep = vars_to_keep, order = vars_to_keep, covariate.labels = labels,
          dep.var.labels = "Natural log of Price", header = FALSE,
          add.lines = list(condo_fe, year_month_fe), label = "splitbyaward",
