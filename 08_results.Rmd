...
Table \@ref(tab:naiveresults) reports the regression results from the "basic" hedonic models. The most basic model specified by equation \@ref(eq:naive) suggests that the GM award provides a `r extract_coef(naive_mdl, "award_dummy")` price premium over non-GM properties. However, this model suffers from omitted variable bias and also has a very low predictive power, with a $R^2$ of `r extract_r2(naive_mdl)`. The model specified by equation \@ref(eq:naivewithcontrols), which adds hedonic characteristics, locational controls (4-digit postal code dummies) and condominium/apartment facilities, shows a GM award effect of `r extract_coef(naive_mdl_with_controls, "award_dummy")`. Because this model adds a significant number of variables, the $R^2$ also increases significantly to `r extract_r2(naive_mdl_with_controls)`. After controlling for property market trends using year-month dummies, the premium associated with GM-rated properties changes to `r extract_coef(naive_mdl_with_controls_ymfe, "award_dummy")`.
...
Finally, a placebo DID model is also run, to ensure that there are no unobserved quality changes that coincide with the announcement of the GM award. The specification is described in the previous section. Running the model on `r format(summary(placebo_mdl)$N, big.mark = ",", scientific = FALSE)` resale transactions shows a positive but insignificant DID coefficient. This result indicates that there is no issue with GM announcements coinciding with unobserved changes in quality of the property.
